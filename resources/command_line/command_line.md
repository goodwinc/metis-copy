...
```
# Syntax: df
$ df -h (human readable. Will report values in one of the 3 below based on which is best)
$ df -k (report values in Kb)
$ df -m (report values in Mb)
$ df -g (report values in Gb)
```
...
$ find ~ git
$ find . "*.pdf" (finds PDF files in current and child directories)
```
...
* `:w`: save file but do not exit editor (not in editing mode)
...
* `Meta` is a special character, it's `alt` on your Mac but you may need to enable it in Terminal preferences.
...
If you want to find a running process (perhaps to get its Process ID, needed to kill it or monitor it), you can do it pretty easily with this handy command:
```
...
### Recommended add-ons
For those who want a more customizable experience using shell commands, here are the packages/add-ons we recommend.

#### Oh-my-zsh
This add-on will customize your z-shell (zsh). Z-shell is an alternative to Bourne Again SHell (bash) which is pre-installed on every Mac. 99% of commands that you can run on bash will work without having to modify them for zsh.

Benefits of oh-my-zsh and zsh:
* plugins - easy installation of plugins to simplify your life, e.g, git, ruby, etc.
* themes - change the boring white background to something with a little more pizzaz!
* autosuggestions - this is the best reason to get oh-my-zsh. It will give you a slightly transparent view of previous commands that match what you're currently typing. See [here](https://github.com/zsh-users/zsh-autosuggestions) for more details

[Oh-My-Zsh Github page](https://github.com/robbyrussell/oh-my-zsh)

#### Autojump
Do you hate having to write the full path to the directory you're trying to get to? So do I.

Autojump lets you use a shell command to get to the most commonly used directory with your keyword.

For example:
```
# this takes you to the folder you have accessed the most that contains "down"
$ j down
/Users/robert/Downloads
```

Learn more about autojump [here](https://github.com/wting/autojump)
